...
get_EBS_List() {
  ...
  ebs_backup_list=$(aws ec2 describe-volumes $ebs_selection_string --output text --query 'Volumes[*].VolumeId')
...
create_EBS_Snapshot_Tags() {
  ...
  if [[ -n $snapshot_tags ]]; then
    ...
    aws_ec2_create_tag_result=$(aws ec2 create-tags --resources $ec2_snapshot_resource_id $tags_argument --output text 2>&1)
  fi
}
...
purge_EBS_Snapshots() {
  ...
  snapshot_purge_allowed=$(aws ec2 describe-snapshots --filters Name=tag:PurgeAllow,Values=true --output text --query 'Snapshots[*].SnapshotId')
  ...
  for snapshot_id_evaluated in $snapshot_purge_allowed; do
    ...
    purge_after_fe=$(aws ec2 describe-snapshots --snapshot-ids $snapshot_id_evaluated --output text | grep ^TAGS.*PurgeAfterFE | cut -f 3)
    ...
    if [[ -z $purge_after_fe ]]; then
      ...
      if [[ $purge_after_fe < $current_date ]]; then
        ...
        aws_ec2_delete_snapshot_result=$(aws ec2 delete-snapshot --snapshot-id $snapshot_id_evaluated --output text 2>&1)
      fi
    fi
  done
}
...
for ebs_selected in $ebs_backup_list; do
  ...
  ec2_create_snapshot_result=$(aws ec2 create-snapshot --description $ec2_snapshot_description --volume-id $ebs_selected --output text 2>&1)
  if [[ $? != 0 ]]; then
